...
- **Assign Country Heads to Delhi and Mumbai** — one `UPDATE` each. Without it
  Velocity Gate #5 cannot close: nobody at L2 can approve those centres' discounts.
...
- **#5 EH discount control gate** — **enforced in the portal** (S6, db/029), but not
  closable until Delhi and Mumbai have a `country_head_id`. Only Gurugram has one.
