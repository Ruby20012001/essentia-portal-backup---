...
## ✅ S11 · Hiring (2026-09-11)
- **Hiring** `/hr` — open seats, candidates, rounds, a question bank and
  scorecards (db/049, `docs/hiring.md`). The screen was a placeholder; it is now
  the module. Built the same blocker in that db/029 and db/030 both hit: every
  fixture account was outside HR, so the module read as broken rather than
  fenced until a dev HR account existed.
- Still open on S11: the **Keka** half. Hiring somebody does not yet create
  their employee record, and no round reaches anybody's calendar.
...
- **Phase 2 (Weeks 6–10, integrations):** S7 BD Pipeline (HubSpot), S8 COO, S9 Procurement (TranZact), S10 Factory HOD, S11 Keka sync (the hiring half is done), S13 API Health, S14 Vendor.
