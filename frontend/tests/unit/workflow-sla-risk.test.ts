...
import { computeSlaRisk } from "@/lib/services/workflow-sla-risk";

/**
 * Deterministic SLA-breach risk. Pure — the level is a function of the
 * deadlines alone, so it can be pinned exactly.
 *
 * These tests were previously carried in workflow-advisory.test.ts alongside
 * the AI advisory's contract; the advisory has been removed and this model,
 * which never depended on it, stands on its own.
 */
